...
export function runSelfUpdatePipeline(improvement) {
  if (!ST.key) {
    toast('Anthropic API key required', 3000)
    return
  }
  if (!ST.ghToken || !ST.ghUser || !ST.ghRepo) {
    toast('GitHub credentials required', 3500)
    return
  }
  ST._building = true
  ...
  row.innerHTML = '<div class="abub">' + html + '</div>'
  s.appendChild(row)
  scrollBot()
  ...
  _nativeFetch(rawUrl)
    ...
    .then(function () {
      ...
      row2.className = 'mrow asst'
      row2.innerHTML =
        '<div class="awrap"><div class="aav">\u26A1</div><div class="abub">\u2705 <strong>Builder updated!</strong> Reloading in 5 seconds\u2026</div></div>'
      s2.appendChild(row2)
      scrollBot()
      setTimeout(function () {
        location.reload()
    ...
    .catch(function (e) {
      ...
      for (var i = 0; i < 4; i++) {
        var el = $(pid + '-s' + i)
        if (el && el.className.indexOf('s-active') >= 0) {
          step = i
          break
        }
      }
      ...
      row3.className = 'mrow asst'
      row3.innerHTML =
        '<div class="awrap"><div class="aav">\u26A1</div><div class="abub">\u274C Self-update failed: ' +
        esc(scrubKeys(String(e.message || e))) +
        '</div></div>'
      s3.appendChild(row3)
      scrollBot()
    })
...
export function selfUpdateBuilder(improvement) {
  ...
  _nativeFetch(rawUrl)
    ...
    .finally(function () {
      if (btn) {
        btn.disabled = false
        btn.textContent = '\uD83D\uDD27 Update The Builder'
      }
    })
}
