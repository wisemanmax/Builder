...
export function runWebsite2Pipeline(prompt, existingApp, customName, images) {
  ...
  p.then(function () {
    ...
    var reconMsg =
      'Analyze this website/site description and extract all content, navigation, branding, and structure. Build a CONTENT_MANIFEST of every stat, name, number, and claim found:\n\n' +
      prompt
    if (images && images.length)
      reconMsg +=
        ...
        ' screenshot' +
        (images.length > 1 ? 's' : '') +
        ' attached \u2014 analyze the visual design, layout, colors, typography, and content from these images]'
    ...
    return retryStep(
      ...
      .catch(function (e) {
        ...
        reconJSON =
          ...
          appName +
          '","description":"' +
          prompt.slice(0, 200).replace(/"/g, '\\"') +
          '","contentManifest":{"stats":[],"names":[],"claims":[],"urls":[]}}'
    ...
    .then(function () {
      ...
      return retryStep(
        function () {
          return _raw(
            brandSys,
            'Extract brand tokens from the recon data. Scan for fonts, colors, border-radius, shadows, and gradients. If not found in CSS, derive from brand name + industry context. Never default to Inter.',
            3000,
            images
          )
        },
        ...
        .then(function (raw) {
          brandJSON = raw
          updatePS(pid, 1, 'done', 'Brand tokens locked \u2713')
          _persistProgress(1)
    ...
    .then(function () {
      ...
      return retryStep(
        function () {
          return _raw(
            structSys,
            'Create the structural map for this site based on the recon data above. Only include pages found in the real site navigation — do not invent pages.',
            4000,
            null
          )
        },
        ...
        .then(function (raw) {
          structureJSON = raw
          updatePS(pid, 2, 'done', 'Structure mapped \u2713')
          _persistProgress(2)
    ...
    .then(function () {
      ...
      return retryStep(
        function () {
          return _raw(
            designSys,
            'Make all design decisions using the brand tokens provided. Use BRAND_TOKENS colors verbatim. Do not override with generic palettes. Inter/Roboto/Arial are banned.',
            4000,
            images
          )
        },
        ...
        .then(function (raw) {
          designJSON = raw
          updatePS(pid, 3, 'done', 'Design system defined \u2713')
          _persistProgress(3)
    ...
    .then(function () {
      ...
      if (images && images.length)
        buildMsg +=
          ...
          ' screenshot' +
          (images.length > 1 ? 's' : '') +
          ' attached \u2014 replicate the visual design as closely as possible]'
      ...
      if (activeThought) {
        ...
        if (!existingApp && activeThought.brief) {
          buildMsg =
            'BUILD THIS WEBSITE: ' +
            (activeThought.brief.name || customName || 'My Site') +
      ...
      return _buildStream(
        ...
        function (type, text) {
          if (type === 'text') {
            ...
            updatePS(pid, 4, 'active', 'Building\u2026 ' + Math.round(charCount / 1000) + 'k chars')
    ...
    .then(function (code) {
      ...
      function runValidationPass() {
        ...
        return retryStep(
          function () {
            return _audit(currentCode, auditSys)
          },
          2,
          'Audit'
        )
          ...
          .then(function (result) {
            ...
            if (allIssues.length > 0) {
              ...
              return retryStep(
                function () {
                  return _multiTurn(fixSys, repairHistory)
                },
                2,
                'Fix'
              )
    ...
    .then(function () {
      ...
      if (hasGitHub) {
        ...
        return retryStep(
          function () {
            return ghGetFileSha(appPath, branchName).then(function (existingSha) {
              return ghPushFile(
                appPath,
                v2,
                (existingApp ? 'Update' : 'Add') + ' ' + appName + ' [website2]',
                branchName,
                existingSha
              )
            })
          ...
          .then(function () {
            updatePS(pid, 8, 'done', 'Pushed to branch \u2713')
            _persistProgress(8)
          })
          .catch(function (e) {
