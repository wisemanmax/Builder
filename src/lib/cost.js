...
export function costCardHTML(cost) {
  ...
  if (cost.totalCacheRead > 0) {
    ...
    cacheSavings =
      '<div class="cost-cache-savings">Cache saved ' + _fmtTokens(savedTokens) + ' tokens (' + _fmt(saved) + ')</div>'
  }
  ...
  return (
    ...
    '<div class="cost-total-amt">' +
    _fmt(cost.rawCost) +
    '</div>' +
    '</div>' +
    '<div class="cost-total-row cost-markup-row">' +
    '<div class="cost-total-label">User Price (' +
    cost.markup +
    'x)</div>' +
    '<div class="cost-total-amt cost-user-price">' +
    _fmt(cost.userPrice) +
    '</div>' +
    '</div>' +
    '<div class="cost-total-row cost-margin-row">' +
    '<div class="cost-total-label">Margin</div>' +
    '<div class="cost-total-amt cost-margin">' +
    _fmt(cost.userPrice - cost.rawCost) +
    '</div>' +
    '</div>' +
    cacheSavings +
    '<div class="cost-meta">' +
    _fmtTokens(cost.totalInput) +
    ' input + ' +
    _fmtTokens(cost.totalOutput) +
    ' output = ' +
    _fmtTokens(cost.totalInput + cost.totalOutput) +
    ' total tokens' +
    '</div>' +
    '</div></div>'
  )
...
export function costSummaryHTML(costs) {
  ...
  for (var j = costs.length - 1; j >= 0; j--) {
    ...
    var dateStr = dt
      ...
      : ''
    rows +=
      '<div class="cost-hist-row">' +
      '<span class="cost-hist-date">' +
      dateStr +
      '</span>' +
      '<span class="cost-hist-raw">' +
      _fmt(c.rawCost) +
      '</span>' +
      '<span class="cost-hist-user">' +
      _fmt(c.userPrice) +
      '</span>' +
      '</div>'
  }
  return (
    ...
    '<div class="cost-summary-item"><span class="cost-summary-label">Total Raw Cost</span><span class="cost-summary-val">' +
    _fmt(totalRaw) +
    '</span></div>' +
    '<div class="cost-summary-item"><span class="cost-summary-label">Total User Price</span><span class="cost-summary-val">' +
    _fmt(totalUser) +
    '</span></div>' +
    '<div class="cost-summary-item"><span class="cost-summary-label">Total Margin</span><span class="cost-summary-val">' +
    _fmt(totalUser - totalRaw) +
    '</span></div>' +
    '<div class="cost-summary-item"><span class="cost-summary-label">Builds</span><span class="cost-summary-val">' +
    costs.length +
    '</span></div>' +
    '</div>' +
