export function runLocalChecks(code) {
  ...
  try {
    ...
    add('Responsive', 'has-media-queries', 'Has media queries',
      hasMediaQueries, 'No media queries found — app will not adapt to different screen sizes')
    add('Responsive', 'multiple-breakpoints', 'Has multiple breakpoints (' + mediaQueryCount + ' found)',
      mediaQueryCount >= 2, 'Need at least 2 breakpoints for mobile/tablet/desktop adaptation')
    add('Responsive', 'has-mobile-breakpoint', 'Has mobile/tablet breakpoint',
      /(@media[^{]*max-width\s*:\s*(4[0-9]{2}|5[0-9]{2}|6[0-9]{2}|7[0-9]{2}|8[0-4][0-9])px)|(@media[^{]*min-width\s*:\s*(3[2-9][0-9]|4[0-9]{2}|5[0-9]{2}|6[0-9]{2}|7[0-9]{2})px)/.test(code),
      'No breakpoint found for mobile/tablet range (320-849px)')
    add('Responsive', 'no-fixed-widths', 'No problematic fixed widths',
    ...
    add('Responsive', 'uses-flex-or-grid', 'Uses Flexbox or Grid layout',
      /display\s*:\s*(flex|grid)/.test(code), 'No flex/grid layouts found — layout may not adapt well')
    add('Responsive', 'responsive-containers', 'Containers use relative sizing',
    ...
    add('Responsive', 'responsive-typography', 'Uses fluid or responsive typography',
    ...
    add('Responsive', 'touch-friendly-inputs', 'Inputs are touch-friendly',
