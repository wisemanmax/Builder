...
export function uniqueSlug(name) {
  ...
  var id = base
  var n = 2
  while (ST.apps.some(function (a) { return a.id === id })) { id = base + '-' + n; n++ }
  return id
}

export function grad(ci) { return GRADS[((ci || 0) % GRADS.length + GRADS.length) % GRADS.length] || GRADS[0] }
...
export function toast(msg, ms) {
  ...
  toast._t = setTimeout(function () { el.classList.remove('show') }, ms)
}

export function showScreen(id) {
  var screens = document.querySelectorAll('.screen')
  for (var i = 0; i < screens.length; i++) screens[i].classList.remove('active')
  if (id) { var s = $('s-' + id); if (s) s.classList.add('active') }
}

export function fmtDate(iso) {
  if (!iso) return '\u2014'
  try { return new Date(iso).toLocaleString(undefined, { month: 'short', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit' }) } catch (e) { return iso }
}
...
export function autoName(prompt) {
  var stop = ['a', 'an', 'the', 'with', 'and', 'for', 'to', 'of', 'that', 'in', 'on', 'at', 'my', 'your', 'build', 'make', 'create', 'app']
  return prompt.split(/\s+/).filter(function (w) { return w.length > 2 && stop.indexOf(w.toLowerCase()) === -1 }).slice(0, 3).map(function (w) { return w.charAt(0).toUpperCase() + w.slice(1).toLowerCase() }).join(' ') || 'My App'
}

export function autoResize(el) { el.style.height = 'auto'; el.style.height = Math.min(el.scrollHeight, 120) + 'px' }

export function autoResizeSe(el) { el.style.height = 'auto'; el.style.height = Math.min(el.scrollHeight, 100) + 'px' }
...
export function copyToClipboard(text, label) {
  if (navigator.clipboard && window.isSecureContext) {
    navigator.clipboard.writeText(text).then(function () { toast((label || 'Text') + ' copied!') }).catch(function () { fallbackCopy(text, label) })
