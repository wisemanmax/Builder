...
export function renderProfilesSettings() {
  ...
  for (var i = 0; i < ST.profiles.length; i++) {
    ...
    var prefCount =
      (prefs.positivePatterns || []).length +
      (prefs.negativePatterns || []).length +
      (prefs.designPrefs || []).length +
      (prefs.functionalPrefs || []).length
    ...
    if (hasOrg) {
      ...
      html +=
        ...
        esc((p.orgProfile.vision || '').slice(0, 120)) +
        (p.orgProfile.vision.length > 120 ? '\u2026' : '') +
        '</div>'
      html +=
        '<button class="prof-btn" data-action="edit-org" data-id="' + p.id + '">\u270F\uFE0F Edit Profile</button>'
      html += '</div>'
    } else {
      html +=
        '<button class="prof-btn primary" data-action="setup-org" data-id="' +
        p.id +
    ...
    if (ruleCount > 0) {
      ...
      if (gr.mustRules && gr.mustRules.length) {
        ...
        for (var m = 0; m < gr.mustRules.length; m++) {
          html +=
            '<div class="prof-rule-item"><span>' +
            esc(gr.mustRules[m]) +
            '</span>' +
            '<button class="prof-rule-rm" data-action="rm-rule" data-id="' +
            p.id +
            '" data-cat="mustRules" data-idx="' +
            m +
            '">\u2715</button></div>'
        }
      }
      if (gr.mustNotRules && gr.mustNotRules.length) {
        ...
        for (var n = 0; n < gr.mustNotRules.length; n++) {
          html +=
            '<div class="prof-rule-item"><span>' +
            esc(gr.mustNotRules[n]) +
            '</span>' +
            '<button class="prof-rule-rm" data-action="rm-rule" data-id="' +
            p.id +
            '" data-cat="mustNotRules" data-idx="' +
            n +
            '">\u2715</button></div>'
        }
      }
      if (gr.niceToHave && gr.niceToHave.length) {
        ...
        for (var nh = 0; nh < gr.niceToHave.length; nh++) {
          html +=
            '<div class="prof-rule-item"><span>' +
            esc(gr.niceToHave[nh]) +
            '</span>' +
            '<button class="prof-rule-rm" data-action="rm-rule" data-id="' +
            p.id +
            '" data-cat="niceToHave" data-idx="' +
            nh +
            '">\u2715</button></div>'
        }
      }
    ...
    html +=
      '<select class="prof-rule-select" data-id="' +
      p.id +
      '"><option value="mustRules">Must Do</option><option value="mustNotRules">Must Not</option><option value="niceToHave">Nice to Have</option></select>'
    html +=
      '<input class="fi prof-rule-input" data-id="' + p.id + '" placeholder="Add a rule\u2026" autocomplete="off">'
    ...
    if (prefCount > 0) {
      ...
      if (prefs.positivePatterns && prefs.positivePatterns.length) {
        ...
        for (var pp = 0; pp < prefs.positivePatterns.length; pp++)
          html += '<div class="prof-insight">\u2705 ' + esc(prefs.positivePatterns[pp]) + '</div>'
      }
      if (prefs.negativePatterns && prefs.negativePatterns.length) {
        html += '<div class="prof-rule-cat">Dislikes</div>'
        for (var np = 0; np < prefs.negativePatterns.length; np++)
          html += '<div class="prof-insight">\u274C ' + esc(prefs.negativePatterns[np]) + '</div>'
      }
      if (prefs.designPrefs && prefs.designPrefs.length) {
        html += '<div class="prof-rule-cat">Design Prefs</div>'
        for (var dp = 0; dp < prefs.designPrefs.length; dp++)
          html += '<div class="prof-insight">\uD83C\uDFA8 ' + esc(prefs.designPrefs[dp]) + '</div>'
      }
      if (prefs.functionalPrefs && prefs.functionalPrefs.length) {
        html += '<div class="prof-rule-cat">Functional Prefs</div>'
        for (var fp = 0; fp < prefs.functionalPrefs.length; fp++)
          html += '<div class="prof-insight">\u2699\uFE0F ' + esc(prefs.functionalPrefs[fp]) + '</div>'
      }
      html +=
        '<div class="prof-insight-meta">Last analyzed: ' +
        (prefs.lastAnalyzedAt ? new Date(prefs.lastAnalyzedAt).toLocaleDateString() : 'never') +
        ' \u00B7 ' +
        (prefs.feedbackCount || 0) +
        ' entries</div>'
      html += '</div>'
    } else {
      html +=
        '<div class="prof-detail" style="color:rgba(255,255,255,.3)">No insights yet \u2014 ' +
        (fbCount < 3 ? 'need at least 3 feedback entries' : 'click Refresh to analyze') +
        '</div>'
    }
    html +=
      '<button class="prof-btn" data-action="refresh-insights" data-id="' +
      p.id +
      '" ' +
      (fbCount < 3 ? 'disabled' : '') +
    ...
    // Delete
    html +=
      '<button class="prof-btn danger" data-action="delete" data-id="' +
      p.id +
...
export function initProfilesSettings() {
  ...
  container.addEventListener('click', function (e) {
    ...
    if (action === 'rm-rule') {
      ...
      var p2 = null
      for (var j = 0; j < ST.profiles.length; j++) {
        if (ST.profiles[j].id === id) {
          p2 = ST.profiles[j]
          break
        }
      }
