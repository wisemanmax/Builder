...
export function initThoughtsFolder() {
  ...
  $('thoughts-overlay').addEventListener('click', function (e) { if (e.target.id === 'thoughts-overlay') closeThoughtsFolder() })
...
export function openThoughtsFolder() {
  ...
  tabsEl.innerHTML = '<div class="tpl-cat active" data-sort="date-new">Newest</div>'
...
function renderThoughtCards(sortBy) {
  ...
  if (sortBy === 'date-new') {
    thoughts.sort(function (a, b) { return (b.updatedAt || b.createdAt || '').localeCompare(a.updatedAt || a.createdAt || '') })
  } else if (sortBy === 'date-old') {
    thoughts.sort(function (a, b) { return (a.updatedAt || a.createdAt || '').localeCompare(b.updatedAt || b.createdAt || '') })
    ...
    thoughts.sort(function (a, b) { return (a.name || '').localeCompare(b.name || '') })
  }
  ...
  for (var i = 0; i < thoughts.length; i++) {
    ...
    var prompt = (t.originalPrompt || '').length > 80 ? t.originalPrompt.slice(0, 80) + '\u2026' : (t.originalPrompt || '')
    var date = t.updatedAt ? fmtDate(t.updatedAt) : (t.createdAt ? fmtDate(t.createdAt) : '')
    html += '<div class="tf-card" onclick="openThoughtDetail(\'' + esc(t.id) + '\')">'
      + '<div class="tf-card-top">'
      + '<div class="tf-card-icon">\uD83D\uDCAD</div>'
      + '<div class="tf-card-status ' + esc(t.status || 'draft') + '">' + esc(t.status || 'draft') + '</div>'
      + '</div>'
      + '<div class="tf-card-name">' + esc(t.name || 'Untitled') + '</div>'
      + '<div class="tf-card-prompt">' + esc(prompt) + '</div>'
      + '<div class="tf-card-meta">'
      + '<span>' + msgCount + ' message' + (msgCount !== 1 ? 's' : '') + '</span>'
      + '<span>\u00B7</span>'
      + '<span>Round ' + (t.rounds || 1) + '/5</span>'
      + '<span>\u00B7</span>'
      + '<span>' + esc(date) + '</span>'
      + '</div>'
      + '</div>'
