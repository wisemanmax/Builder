...
export function renderOrgProgress(round) {
  ...
  $('org-ts-sub').textContent =
    'Round ' + Math.min(round, _orgState.maxRounds) + ' of ' + _orgState.maxRounds + ' \u00B7 ' + label
}

function scrollOrgBot() {
  var el = $('org-think-scroll')
  setTimeout(function () {
...
function addOrgMsg(cfg) {
  ...
  if (cfg.type === 'user') {
    ...
    row.innerHTML =
      '<div class="awrap"><div class="aav" style="background:var(--g2)">\uD83C\uDFE2</div><div class="tbub"><div class="td"></div><div class="td"></div><div class="td"></div></div></div>'
  } else if (cfg.type === 'ai') {
    row.className = 'mrow asst'
    row.innerHTML =
      '<div class="awrap"><div class="aav" style="background:var(--g2)">\uD83C\uDFE2</div><div class="abub">' +
      esc(cfg.text) +
      '</div></div>'
...
export function sendOrgMsg() {
  ...
  apiMessages.push({
    ...
    content:
      ...
      _orgState.maxRounds +
      '\nPhase: ' +
      ORG_ROUND_LABELS[Math.min(_orgState.round, _orgState.maxRounds) - 1] +
      '\nOrganization name: ' +
      _orgState.originalPrompt +
      '\n\n' +
      text,
  })
...
function _saveOrgProfile(profileData, rulesData) {
  var profile = null
  for (var i = 0; i < ST.profiles.length; i++) {
    if (ST.profiles[i].id === _orgState.profileId) {
      profile = ST.profiles[i]
      break
    }
  }
...
function renderOrgSummary(profileData, rulesData) {
  ...
  if (profileData.vision)
    html +=
      '<h4>Vision</h4><p style="font-size:12px;color:rgba(255,255,255,.7);line-height:1.6;padding:0 16px">' +
      esc(profileData.vision) +
      '</p>'
  ...
  if (profileData.brandIdentity) {
    ...
    if (profileData.brandIdentity.theme)
      html += '<span class="ts-tag">' + esc(profileData.brandIdentity.theme) + ' theme</span>'
    if (profileData.brandIdentity.accentColor)
      html += '<span class="ts-tag">' + esc(profileData.brandIdentity.accentColor) + '</span>'
    if (profileData.brandIdentity.tone)
      html += '<span class="ts-tag">' + esc(profileData.brandIdentity.tone) + '</span>'
    html += '</div>'
  }
...
export function refineOrgThink() {
  ...
  _orgState.conversation.push({
    role: 'user',
    text: '[Round reset to ' + _orgState.round + '. Previous profile was rejected. Continue refining from this round.]',
    round: _orgState.round,
  })
  addOrgMsg({ type: 'ai', text: "No problem! Let's refine. What would you like to change?" })
  _orgState.conversation.push({
    role: 'ai',
    text: "Let's refine. What would you like to change?",
    round: _orgState.round,
  })
  scrollOrgBot()
}
...
export function initOrgThinkSheet() {
  ...
  if (sheet)
    sheet.addEventListener('click', function (e) {
      if (e.target.id === 'org-think-sheet') closeOrgThink()
    })
  var inp = $('org-think-input')
  if (inp) {
