...
export function renderMarketMap(container) {
  ...
  for (var j = 0; j < COMPETITORS.length; j++) {
    ...
    html += '<span style="font-family:var(--fm);font-size:11px;color:var(--mie-text-muted)">(' + c.scores[dim.xKey] + ', ' + c.scores[dim.yKey] + ')</span>'
    html += '</div>'
  }
  html += '</div>'

  // Whitespace note
  html += '<div style="margin-top:12px;padding:10px 14px;background:rgba(170,255,0,0.05);border:1px solid rgba(170,255,0,0.15);border-radius:8px;font-size:12px;color:rgba(170,255,0,0.7)">'
...
function buildSVG(dim) {
  ...
  svg += '<rect x="' + wsX + '" y="' + wsY + '" width="' + wsW + '" height="' + wsH + '" fill="rgba(170,255,0,0.04)" stroke="rgba(170,255,0,0.15)" stroke-dasharray="6,4" rx="4"/>'
  svg += '<text x="' + (wsX + wsW / 2) + '" y="' + (wsY + 18) + '" text-anchor="middle" fill="rgba(170,255,0,0.3)" font-size="10" font-family="var(--fh)" font-weight="700">WHITESPACE</text>'
  ...
  for (var g = 0; g <= 100; g += 25) {
    ...
    // X labels
    svg += '<text x="' + gx + '" y="' + (vh - 14) + '" text-anchor="middle" fill="rgba(255,255,255,0.25)" font-size="10" font-family="var(--fm)">' + g + '</text>'
    // Y labels
    svg += '<text x="' + (pad.left - 10) + '" y="' + (gy + 4) + '" text-anchor="end" fill="rgba(255,255,255,0.25)" font-size="10" font-family="var(--fm)">' + g + '</text>'
  }

  // Axes
  svg += '<line x1="' + pad.left + '" y1="' + (pad.top + plotH) + '" x2="' + (pad.left + plotW) + '" y2="' + (pad.top + plotH) + '" stroke="rgba(255,255,255,0.15)" stroke-width="1.5"/>'
  svg += '<line x1="' + pad.left + '" y1="' + pad.top + '" x2="' + pad.left + '" y2="' + (pad.top + plotH) + '" stroke="rgba(255,255,255,0.15)" stroke-width="1.5"/>'

  // Axis labels
  svg += '<text x="' + (pad.left + plotW / 2) + '" y="' + (vh - 2) + '" text-anchor="middle" fill="rgba(255,255,255,0.4)" font-size="11" font-family="var(--fh)" font-weight="600">' + dim.xLabel + '</text>'
  ...
  for (var i = 0; i < COMPETITORS.length; i++) {
    ...
    if (c.isSelf) {
      ...
      svg += '<circle cx="' + cx + '" cy="' + cy + '" r="' + (r + 6) + '" fill="none" stroke="rgba(255,60,172,0.2)" stroke-width="2" stroke-dasharray="4,3"/>'
      svg += '<circle cx="' + cx + '" cy="' + cy + '" r="' + r + '" fill="' + c.color + '" opacity="0.85" data-id="' + c.id + '" class="mie-bubble" style="cursor:pointer;filter:drop-shadow(0 0 8px rgba(255,60,172,0.4))"/>'
    } else {
      svg += '<circle cx="' + cx + '" cy="' + cy + '" r="' + r + '" fill="' + c.color + '" opacity="0.7" data-id="' + c.id + '" class="mie-bubble" style="cursor:pointer"/>'
    }

    // Label
    var labelY = cy - r - 6
    if (labelY < pad.top + 10) labelY = cy + r + 14
    svg += '<text x="' + cx + '" y="' + labelY + '" text-anchor="middle" fill="' + (c.isSelf ? '#FF3CAC' : 'rgba(255,255,255,0.6)') + '" font-size="10" font-family="var(--fh)" font-weight="' + (c.isSelf ? '800' : '600') + '">' + c.name + '</text>'
  }
