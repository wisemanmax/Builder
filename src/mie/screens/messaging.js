...
var MESSAGING_RECS = [
  {
    ...
    description:
      "Target borrowers who have been rejected or expect rejection from major lenders. Emphasize GradBridge's flexible underwriting and inclusive criteria.",
    status: 'approved',
  },
  {
    id: 'msg-2',
    ...
    description:
      'When a competitor offers lower rates but has strict approval criteria, shift the conversation to approval probability and support quality.',
    status: 'approved',
  },
  {
    id: 'msg-3',
    ...
    description:
      'For the large segment of borrowers without cosigner access, position GradBridge as the lender that believes in the individual borrower.',
    status: 'approved',
  },
  {
    id: 'msg-4',
    ...
    description:
      'When competitor sentiment drops in specific categories, create messaging that addresses those exact pain points without naming competitors directly.',
    status: 'pending',
  },
  {
    id: 'msg-5',
    ...
    description:
      'First-time borrowers need reassurance. Lead with transparency and human support to reduce anxiety and build trust before the application.',
    status: 'approved',
  },
  {
    id: 'msg-6',
    ...
    description:
      'For prime borrowers who could go anywhere, compete on rate directly and layer in differentiators like faster approval and better support.',
    status: 'pending',
  },
  {
    id: 'msg-7',
    ...
    description:
      'Non-traditional students face systemic rejection. Position GradBridge as the lender that serves the reality of modern education.',
    status: 'approved',
  },
  {
    id: 'msg-8',
    ...
    description:
      'Graduate borrowers think in ROI terms. Frame the loan as an investment with clear terms and a partner who understands the payoff timeline.',
    status: 'pending',
  },
]
...
export function renderMessaging(container, signal) {
  ...
  html +=
    'AI-generated messaging recommendations based on current competitive conditions. Approve or reject to add to the experiment queue.'
  html += '</div>'

  // Stats
  var approved = MESSAGING_RECS.filter(function (r) {
    return r.status === 'approved'
  }).length
  var pending = MESSAGING_RECS.filter(function (r) {
    return r.status === 'pending'
  }).length
  html += '<div class="mie-kpi-row" style="margin-bottom:20px">'
  html +=
    '<div class="mie-kpi"><div class="mie-kpi-val" style="color:#00E676">' +
    approved +
    '</div><div class="mie-kpi-label">Approved</div></div>'
  html +=
    '<div class="mie-kpi"><div class="mie-kpi-val" style="color:#FFD600">' +
    pending +
    '</div><div class="mie-kpi-label">Pending Review</div></div>'
  html +=
    '<div class="mie-kpi"><div class="mie-kpi-val" style="color:#7C4DFF">' +
    MESSAGING_RECS.length +
    '</div><div class="mie-kpi-label">Total Variants</div></div>'
  html += '</div>'
  ...
  for (var i = 0; i < MESSAGING_RECS.length; i++) {
    ...
    html +=
      '<div style="font-size:12px;color:var(--mie-text-secondary);margin-top:6px;line-height:1.5">' +
      rec.description +
      '</div>'
    html += '<div class="mie-msg-status">'
    html +=
      '<button class="mie-msg-status-btn' +
      (rec.status === 'approved' ? ' approved' : '') +
      '" data-action="approve" data-idx="' +
      i +
      '">Approve</button>'
    html +=
      '<button class="mie-msg-status-btn' +
      (rec.status === 'rejected' ? ' rejected' : '') +
      '" data-action="reject" data-idx="' +
      i +
      '">Reject</button>'
