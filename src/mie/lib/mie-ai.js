...
import { getCompetitors, getSegments, getSentimentScores, getSampleReviews, getAlerts,
  setCachedCompetitors, setCachedAlerts, setCachedSegments, setCachedSentiment } from './mie-data.js'
...
var SYS_REFRESH = 'You are a student lending market intelligence analyst. You update competitive data for GradBridge, a student loan fintech. Return ONLY valid JSON matching the exact schema provided. No markdown, no commentary.'

export function refreshCompetitors(key) {
  var current = getCompetitors()
  var schema = JSON.stringify(current.map(function (c) {
    return { id: c.id, name: c.name, scores: c.scores, products: c.products.map(function (p) {
      return { name: p.name, fixedAprMin: p.fixedAprMin, fixedAprMax: p.fixedAprMax, variableAprMin: p.variableAprMin, variableAprMax: p.variableAprMax }
    }) }
  }))
  var prompt = 'Here is the current competitor data for the private student loan market:\n' + schema + '\n\nGenerate updated, realistic competitor data reflecting current 2026 market conditions. Update scores (0-100) and APR ranges based on realistic market movements. Keep the same structure. Return the full competitor array with ALL original fields preserved, just update scores and product APR values. Return as a JSON array.'
  ...
  return callGPT(key, SYS_REFRESH, prompt, 6000).then(function (raw) {
    ...
    var merged = current.map(function (orig) {
      var updated = parsed.find(function (u) { return u.id === orig.id })
...
export function refreshSegments(key) {
  ...
  var prompt = 'Here are current underserved borrower segments for the student lending market:\n' + JSON.stringify(current.map(function (s) {
    return { id: s.id, name: s.name, opportunityScore: s.opportunityScore, rejectionRate: s.rejectionRate, gbApprovalProbability: s.gbApprovalProbability, marketSizeIndex: s.marketSizeIndex, competitorGap: s.competitorGap }
...
export function refreshAll(key) {
  return Promise.all([
    refreshCompetitors(key).catch(function (e) { return { error: 'competitors: ' + e.message } }),
    refreshSentiment(key).catch(function (e) { return { error: 'sentiment: ' + e.message } }),
    refreshSegments(key).catch(function (e) { return { error: 'segments: ' + e.message } }),
    refreshAlerts(key).catch(function (e) { return { error: 'alerts: ' + e.message } }),
  ]).then(function (results) {
    var errors = results.filter(function (r) { return r && r.error })
    if (errors.length > 0) {
      throw new Error(errors.map(function (e) { return e.error }).join('; '))
    }
    return results
  })
}
...
export function refreshSection(key, section) {
  switch (section) {
    case 'competitors': return refreshCompetitors(key)
    case 'sentiment': return refreshSentiment(key)
    case 'segments': return refreshSegments(key)
    case 'alerts': return refreshAlerts(key)
