...
export function renderRefreshBar(container, onRefreshDone) {
  ...
  if (!hasKey) {
    ...
    for (var i = 0; i < SECTIONS.length; i++) {
      html += '<button class="mie-refresh-dropdown-item" data-section="' + SECTIONS[i].id + '">Refresh ' + SECTIONS[i].label + '</button>'
    }
    html += '</div></div>'
  ...
  for (var j = 0; j < sectionBtns.length; j++) {
    sectionBtns[j].addEventListener('click', function () {
...
function doRefresh(barContainer, section, onDone) {
  ...
  status.innerHTML = '<span class="mie-spinner"></span> ' + (section ? 'Refreshing ' + section + '...' : 'Refreshing all intelligence data...')
  ...
  promise.then(function () {
    ...
    status.innerHTML = '&#x2713; ' + (section ? section.charAt(0).toUpperCase() + section.slice(1) + ' updated' : 'All data updated')
